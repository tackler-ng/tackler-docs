...
| Optional external id for transaction. +
 +
If present, this must be non-empty, single line. The value will be trimmed (begin, end) when read. +
 +
It could be an id from external system, or checksum generated by CSV importer for the imported row. +
 +
The ext-id must be unique, if `kernel.ext-id.unique` is true.
See xref:reference:tackler-toml.adoc#kernel-ext-id[tackler.toml]
for further information. +
Since v26.07.2
