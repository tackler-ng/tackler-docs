...
* If time part is missing then `default-time` defined in xref:reference:tackler-toml.adoc#kernel-timestamp[tackler.toml] is used.
* If Offset is missing then offset based on timezone/offset defined in xref:reference:tackler-toml.adoc#kernel-timestamp-timezone[tackler.toml] is used.
...
by `report.scale.{min, max}` in xref:reference:tackler-toml.adoc#report[tackler.toml].
...
2017-01-01 'Txn with UUID
 ...
 Assets:Cash
----
| Optional tackler's unique identifier for transaction (link:https://en.wikipedia.org/wiki/Universally_unique_identifier[UUID]). +
 +
Transaction UUID is mandatory for all transactions, if audit mode is activated.
See xref:auditing:index.adoc[Accounting Auditing] and xref:reference:tackler-toml.adoc#kernel-audit[tackler.toml]
...
| [[ext-id]]
----
2026-07-12 'Txn with external id
 # ext-id: REF-123456789
 Expenses:Ice_cream 2.12
 Assets:Cash
----
| Optional external id for transaction. +
 +
If present, this is free text, and it must be non-empty, single line. The value will be trimmed (begin, end). +
 +
Typically it's an id from external system, or id generated by CSV importer. +
 +
The ext-id must be unique, if `kernel.ext-id.unique` is true.
See xref:reference:tackler-toml.adoc#kernel-ext-id[tackler.toml] for full details. +
Since v26.08.1
