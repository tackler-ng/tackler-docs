...
* Integrated, native support for  xref:journal:git-storage.adoc[Git SCM based journal storage], normal xref:journal:fs-storage.adoc[filesystem based journal] is also supported.
...
** xref:reports:export-equity.adoc[Equity],
xref:reports:export-identity.adoc[Identity] and
xref:reports:export-accounts.adoc[Accounts] exports
...
xref:usage:txn-filters.adoc#txn-extid[Ext-Id],
