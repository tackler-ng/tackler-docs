...
from 300 000 to 900 000 transactions per second on modern laptop.

**What does it mean to process 300 000 txns per sec?**
...
Journal with a transaction in every minute, every day, for one year - that's journal with about 500 000 transactions.
...
Typically tackler is 2 times faster than ledger (C+\+), 20 times faster than hledger (Haskell)
and 50 times faster than beancount (C++ and Python). At the same time, it has modest memory
footprint, which is 2-4 times smaller than memory usage of those other tools.
